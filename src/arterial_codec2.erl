-module(arterial_codec2).

-moduledoc """
Behaviour implemented by wire-codec modules for `arterial_pool2`'s
backend (NIF-resident raw socket I/O, see `arterial_nif_pool`'s
moduledoc).
...
owned entirely by `arterial_connection2` + `arterial_nif_pool`, and every
...
32 bits for the default `arterial_codec2_default` framing below); pick a
width wide enough that `erlang:unique_integer/1`-derived ids (see
`arterial_client2:new_corr_id/0`) don't collide after truncation for as
long as a request can plausibly still be in flight.
""".

-doc """
Encode `Request` (assigned `CorrId` by `arterial_client2:call/3` or
...
corruption -- on `{error, _}`, `arterial_connection2` logs and drops the
