-module(arterial_client2).

-moduledoc """
Public request API for `arterial_pool2` (`arterial`'s second connection-
pool backend, see `arterial_pool2`'s moduledoc).
...
`arterial_nif_pool:send_and_release/3`) and immediately gives up any
...
pool's public ETS table by whichever `arterial_connection2` worker
...
-spec call(arterial_pool2:name(), term()) -> {ok, arterial:response()} | {error, term()}.
call(Pool, Request) ->
  call(Pool, Request, arterial_pool2:default_timeout_ms(Pool)).

-doc """
Encode and send `Request` on any currently available, non-throttled
connection of `Pool` (tried round-robin, starting from a scheduler-
affine offset to spread contention -- see `arterial_pool2`'s moduledoc),
then block for its reply (matched purely by wire-level correlation id,
see `c:arterial_codec2:decode/1`) or `Timeout` milliseconds, whichever
comes first.

Returns `{error, no_connection}` if every connection is currently
unavailable/throttled, `{error, disconnected}` if the connection that
carried this request dies before a reply arrives (see
`arterial_connection2`'s disconnect-notification path), or
`{error, timeout}` if `Timeout` elapses first either way.

## Examples

```
1> arterial_client2:call(my_pool, {get, <<"key">>}, 5000).
{ok, <<"value">>}
```
""".
-spec call(arterial_pool2:name(), term(), non_neg_integer() | infinity) ->
...
-doc """
Encode and send `Request` on any currently available, non-throttled
connection of `Pool` without waiting for (or expecting) any reply -- the
...
accepted by `arterial_nif_pool:send_and_release/3`, never registering
...
```
1> arterial_client2:cast(my_pool, {log, info, <<"started">>}).
ok
```
""".
-spec cast(arterial_pool2:name(), term()) -> ok | {error, term()}.
cast(Pool, Request) ->
  case arterial_observe:enabled() of
    false ->
...
`arterial_codec2_default`'s framing reserves for it) -- custom
`c:arterial_codec2` implementations with a wider/narrower id field
...
try_send(Pool, ConnID, CorrId, Data, Timeout) ->
  ...
  %% ever be noticed by arterial_connection2 after the write below
  ...
  case arterial_nif_pool:send_and_release(PoolRef, ConnID, [Data]) of
    {ok, _SlotId} ->
      ok;
    {error, _Reason} ->
...
await_reply(Pool, CorrId, Timeout) ->
  receive
    {arterial_reply, CorrId, Reply} ->
      {ok, Reply};
    {arterial_disconnected, Pool, CorrId} ->
      {error, disconnected};
    {arterial_timeout, Pool, CorrId} ->
      {error, timeout}
  after Timeout ->
    ets:delete(arterial_pool2:corr_table(Pool), CorrId),
    {error, timeout}
  end.

%% Scheduler-affine starting offset, then a linear scan of every
%% remaining ConnID (wrapping), skipping any already in `Tried`, any
%% currently unavailable connection (no socket / disconnected /
%% draining for a bounce), and any throttled one.
...
find_candidate(Pool, Size, Tried, Start, Offset) ->
  ...
  case lists:member(ConnID, Tried) of
    ...
    false ->
      case arterial_pool2:is_available(Pool, ConnID) andalso throttle_ok(Pool, ConnID) of
        true  -> ConnID;
        false -> find_candidate(Pool, Size, Tried, Start, Offset + 1)
      end
  end.

throttle_ok(Pool, ConnID) ->
  case arterial_pool2:throttle(Pool) of
    undefined          -> true;
    {Ref, Rate, Burst} -> arterial_throttle2:allow(Ref, ConnID, Rate, Burst)
  end.
