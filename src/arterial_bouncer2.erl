-module(arterial_bouncer2).

-moduledoc """
Periodic `gen_server` that recycles one of `arterial_pool2`'s connections
...
`arterial_connection2:bounce/2` instead.

Started once per pool by `arterial_pool2`'s supervisor only if the
...
disconnect anyway (see `arterial_connection2:bounce/2`).

## Examples

```
1> arterial_bouncer2:start_link(my_pool, 8, 60000, 30000).
{ok,<0.151.0>}
```
""".
-spec start_link(arterial_pool2:name(), pos_integer(), pos_integer(), pos_integer()) ->
  {ok, pid()}.
...
bounce(Pool, ConnID, DrainTimeoutMs) ->
  case conn_pid(Pool, ConnID) of
    {ok, Pid} ->
      try arterial_connection2:bounce(Pid, DrainTimeoutMs) of
        ok               -> ok;
        {error, timeout} ->
...
conn_pid(Pool, ConnID) ->
  Children = supervisor:which_children(arterial_pool2:sup_name(Pool)),
  case lists:keyfind({arterial_connection2, ConnID}, 1, Children) of
