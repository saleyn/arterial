-module(arterial_sweeper2).

-moduledoc """
Periodic `gen_server` that evicts expired in-flight requests from
`arterial_pool2`'s correlation-id ETS table, mirroring
...
Started once per pool by `arterial_pool2`'s supervisor; not meant to be
...
`{arterial_timeout, Pool, CorrId}` (matching `arterial_client2:call/3`'s
...
```
1> arterial_sweeper2:start_link(my_pool, 1000).
{ok,<0.150.0>}
```
""".
-spec start_link(arterial_pool2:name(), pos_integer()) -> {ok, pid()}.
...
sweep(Pool) ->
  ...
  Table = arterial_pool2:corr_table(Pool),
