...
inline int create_connection_timeout_fd(uint64_t timeout_ms)
{
  ...
  if (timer_fd < 0) return -1; // Failed to create timerfd
...
{
  if (timeout_fd >= 0) close(timeout_fd);
}
...
inline int create_connection_timeout_fd(uint64_t timeout_ms)
{
  ...
  int kq = kqueue();
  if (kq < 0) return -1; // Failed to create kqueue
...
struct GenericTimerEntry {
  ...
  GenericTimerEntry(uint64_t timeout_ms)
  {
    ...
    m_expiration = std::chrono::steady_clock::now() + std::chrono::milliseconds(timeout_ms);
  }
  ...
  bool operator>(const GenericTimerEntry& other) const { return m_expiration > other.m_expiration; }
};
...
public:
  ...
  int create_timer(uint64_t timeout_ms)
  {
    ...
    if (timer->m_pipe_read_fd < 0) return -1; // Pipe creation failed
...
private:
  ...
  uint64_t get_next_timer_id() { return m_next_timer_id.fetch_add(1); }
  ...
  void worker_loop()
  {
    while (!m_shutdown_requested.load()) {
      ...
      while (!m_timer_queue.empty()) {
        ...
        if (entry.expiration_time_ms > now_ms) break; // No more expired timers
...
public:
  ...
  GenericTimerRegistry() { GenericTimerManager::instance().initialize(); }

  int create_timer(uint64_t timeout_ms)
  { return GenericTimerManager::instance().create_timer(timeout_ms); }

  void remove_timer(int fd) { GenericTimerManager::instance().remove_timer(fd); }
};
...
inline void close_connection_timeout_fd(int timeout_fd)
{
  if (timeout_fd >= 0) GenericTimerRegistry::instance().remove_timer(timeout_fd);
}
