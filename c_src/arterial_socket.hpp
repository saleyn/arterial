...
inline bool set_int_or_bool_sockopt(
    ErlNifEnv* env, const ERL_NIF_TERM* opts, ERL_NIF_TERM name, Fun f, ERL_NIF_TERM& err)
{
  if (!enif_is_identical(opts[0], name)) return false;
  int value;
  if (get(env, opts[1], value)) goto SET;
  if (enif_is_identical(opts[1], am_true)) {
    value = 1;
    goto SET;
  }
  if (enif_is_identical(opts[1], am_false)) {
    value = 0;
    goto SET;
  }
...
inline bool set_int_or_bool_sockopt(
    ErlNifEnv* env, const ERL_NIF_TERM* opts, ERL_NIF_TERM name, int fd, int opt_class,
    int opt_type, ERL_NIF_TERM& err)
{
  auto f = [=](int v) { return setsockopt(fd, opt_class, opt_type, &v, sizeof(v)); };
  return set_int_or_bool_sockopt(env, opts, name, f, err);
}

// If the {OptName, OptValue} option matches given name, set it
inline bool set_atom_sockopt(
    const ERL_NIF_TERM opt, ERL_NIF_TERM name, int fd, int opt_class, int opt_type,
    ERL_NIF_TERM& err)
{
    ...
    ErlNifEnv* env, const ERL_NIF_TERM* opts, ERL_NIF_TERM name, Fun f, ERL_NIF_TERM& err)
{
  if (!enif_is_identical(opts[0], name)) return false;
  Tuple tup;
  if (!get(env, opts[1], tup)) [[unlikely]] {
    err = name;
    return false;
  }
  auto res = f(tup) == 0;
    ...
    int opt_type, ERL_NIF_TERM& err)
{
  return set_tuple_sockopt<std::tuple<ERL_NIF_TERM, ERL_NIF_TERM>>(
      ...
      [=](auto& tup) {
        ...
        if (!get(env, maddr, mcast_addr) || !get(env, iaddr, if_addr)) return false;
        struct ip_mreq mreq;
...
bool apply_sock_opts(int fd, ErlNifEnv* env, ERL_NIF_TERM options_list, ERL_NIF_TERM& err)
{
  ...
  if (enif_is_empty_list(env, options_list)) return true; // No options to apply
  ...
  while (enif_get_list_cell(env, tail, &head, &tail)) {
    ...
    if (enif_is_atom(env, head)) {
      ...
      if (arterial::set_atom_sockopt(head, am_keepalive, fd, SOL_SOCKET, SO_KEEPALIVE, err) ||
        ...
        enif_get_tuple(env, head, &tuple_arity, &tuple_elements) && tuple_arity == 2 &&
        enif_is_atom(env, tuple_elements[0])) {
      // Handle tuple-based options {Option, Value} - try each one until one matches
      if (arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_keepalive, fd, SOL_SOCKET, SO_KEEPALIVE, err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_nodelay, fd, IPPROTO_TCP, TCP_NODELAY, err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_linger,
              [fd](int v) {
              ...
              err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_reuseaddr, fd, SOL_SOCKET, SO_REUSEADDR, err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_sndbuf, fd, SOL_SOCKET, SO_SNDBUF, err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_rcvbuf, fd, SOL_SOCKET, SO_RCVBUF, err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_recvbuf, fd, SOL_SOCKET, SO_RCVBUF, err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_priority, fd, SOL_SOCKET, SO_PRIORITY, err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_tos, fd, IPPROTO_IP, IP_TOS, err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_user_timeout, fd, IPPROTO_TCP, TCP_USER_TIMEOUT, err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_cork, fd, IPPROTO_TCP, TCP_CORK, err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_quickack, fd, IPPROTO_TCP, TCP_QUICKACK, err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_rcvlowat, fd, SOL_SOCKET, SO_RCVLOWAT, err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_sndlowat, fd, SOL_SOCKET, SO_SNDLOWAT, err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_keepidle, fd, IPPROTO_TCP, TCP_KEEPIDLE, err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_keepintvl, fd, IPPROTO_TCP, TCP_KEEPINTVL, err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_keepcnt, fd, IPPROTO_TCP, TCP_KEEPCNT, err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_multicast_ttl,
              [fd](int v) {
              ...
              err) ||
          arterial::set_int_or_bool_sockopt(
              env, tuple_elements, am_multicast_loop, fd, IPPROTO_IP, IP_MULTICAST_LOOP, err) ||
          arterial::set_tuple_sockopt<std::tuple<bool, int>>(
              env, tuple_elements, am_linger,
              [fd](const std::tuple<bool, int>& tup) {
                struct linger l = {
                    .l_onoff  = std::get<0>(tup) ? 1 : 0,
                    .l_linger = std::get<1>(tup) /* seconds */};
                return setsockopt(fd, SOL_SOCKET, SO_LINGER, &l, sizeof(l));
              },
              err)
          ...
          || set_tuple_sockopt<IP4Tuple>(
                 ...
                 [fd](const auto& arg) {
                   ...
                   return setsockopt(
                       fd, IPPROTO_IP, IP_MULTICAST_IF, &interface_addr, sizeof(interface_addr));
                 },
                 err)
          // Handle {add_membership, {{A,B,C,D}, {E,F,G,H}}} format
          || set_addr_sockopt(
                 env, tuple_elements, am_add_membership, fd, IPPROTO_IP, IP_ADD_MEMBERSHIP, err)
          // Handle {drop_membership, {{A,B,C,D}, {E,F,G,H}}} format
          ||
          set_addr_sockopt(
              env, tuple_elements, am_drop_membership, fd, IPPROTO_IP, IP_DROP_MEMBERSHIP, err)) {
        // One of the options matched and was set successfully
        continue;
      } else {
