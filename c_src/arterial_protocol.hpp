...
struct ProtocolHandler<PROTO_TCP> {
  static int  create_socket() { return socket(AF_INET, SOCK_STREAM, 0); }
  ...
  static bool configure_socket(int fd, bool nodelay)
  {
    ...
    if (nodelay) {
      int flag = 1;
      if (setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, &flag, sizeof(flag)) == -1) return false;
    }
...
struct ProtocolHandler<PROTO_UDP> {
  static int  create_socket() { return socket(AF_INET, SOCK_DGRAM, 0); }
...
struct ProtocolHandler<PROTO_SSL> {
  ...
  static ssize_t read_data(Connection& slot, char* buffer, size_t buffer_size)
  {
    if (slot.ssl) return SSL_read(slot.ssl, buffer, buffer_size);
  ...
  static ssize_t write_data(Connection& slot, const struct iovec* iov, int iov_count)
  {
    ...
    if (iov_count == 1) return SSL_write(slot.ssl, iov[0].iov_base, iov[0].iov_len);
    ...
    for (int i = 0; i < iov_count; ++i) {
      ...
      if (written <= 0) return total_written > 0 ? total_written : written;
      total_written += written;
    }
    return total_written;
  }

  static void cleanup_connection(Connection& slot) { cleanup_slot_ssl(slot); }
};
#endif
...
static int create_socket_for_protocol(ProtocolType protocol)
{
  switch (protocol) {
    ...
    case PROTO_SSL: // SSL uses TCP socket - SSL handshake happens after connection
      return socket(AF_INET, SOCK_STREAM, 0);
    case PROTO_UDP: return socket(AF_INET, SOCK_DGRAM, 0);
    default:        return -1;
  }
}
...
static ProtocolType parse_protocol(ErlNifEnv* env, ERL_NIF_TERM term)
{
  ...
  ProtocolType result;
  if (proto_atom == am_tcp)
    result = PROTO_TCP;
  else if (proto_atom == am_udp)
    result = PROTO_UDP;
  else if (proto_atom == am_ssl)
    result = PROTO_SSL;
  else
    result = static_cast<ProtocolType>(-1);
