-module(arterial_bench2).

-moduledoc """
Throughput/latency micro-benchmark for `arterial_pool2`/`arterial_client2`
(the NIF-resident-I/O backend, see `arterial_pool2`'s moduledoc), driven
...
`sync`/`async`): `arterial_client2:call/2,3` already writes inline in
...
apply either, for the same reason (see `arterial_pool2`'s moduledoc on
...
1> arterial_bench2:bench().
=== arterial_pool2 benchmark ===
pool size:    8
workers:      8
...
```
1> arterial_bench2:bench(#{pool_size => 16, duration_s => 10}).
2> arterial_bench2:bench(#{throttle => #{rate => 1000, burst => 100}}).
```
...
-type opts() :: #{
  ...
  throttle        => undefined | arterial_pool2:throttle_opts(),
  external_server => boolean()
}.

-define(POOL, arterial_bench2_pool).
...
-- from a shell (`arterial_bench2:help().`) or via `make bench2
...
help() ->
  io:format(
    "arterial_bench2:bench(Opts) -- Opts is a map with any of:~n"
    "~n"
    "  pool_size  => pos_integer()  (default: 8)~n"
    "                Number of pooled connections to test_tcp_server~n"
    "                (also the number of arterial_nif_pool stripes, one~n"
    "                connection per stripe -- see arterial_pool2's~n"
    "                moduledoc).~n"
    ...
    "~n"
    "Examples:~n"
    "  1> arterial_bench2:bench().~n"
    "  2> arterial_bench2:bench(#{pool_size => 16, duration_s => 10}).~n"
    "  3> arterial_bench2:bench(#{throttle => #{rate => 1000, burst => 100}}).~n"
    "  4> arterial_bench2:bench(#{backoff => {backoff, 100, 5000}}).~n"
    "~n"
    "  $ make bench2~n"
...
`arterial_pool2` -- safe to call repeatedly from a live shell.
""".
-spec bench(opts()) -> ok.
bench(Opts) ->
...
setup(PoolSize, Throttle, Backoff, ExternalServer) ->
  ...
  try
    {ok, _SupPid} = arterial_pool2:start_link(?POOL, PoolOpts),
...
teardown(Srv) ->
  arterial_pool2:stop(?POOL),
  bench_external_server:stop(Srv).

wait_until_available(Size, Retries) ->
  AllUp = lists:all(fun(ConnID) -> arterial_pool2:is_available(?POOL, ConnID) end,
                     lists:seq(0, Size - 1)),
  case AllUp of
    true ->
      ok;
...
worker_loop(Parent, Deadline, Payload, Acc, Rejected) ->
  case erlang:monotonic_time(millisecond) >= Deadline of
    ...
    false ->
      ...
      case arterial_client2:call(?POOL, {echo, Payload}, 5000) of
        {ok, Payload} ->
...
report(PoolSize, Workers, DurationMs, ElapsedMs, LatenciesUs, Rejected) ->
  ...
  io:format("=== arterial_pool2 benchmark ===~n"),
