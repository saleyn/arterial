...
the same concept applies (`pool_size`, `workers`, `duration`,
...
$ make bench-poolboy   BENCH_OPTS='pool_size=16 duration=10'
```
""".
...
help() ->
  io:format(
    ...
    "  2> poolboy_bench:bench(#{pool_size => 16, duration => 10}).~n"
    "  3> poolboy_bench:bench(#{pool_strategy => lifo}).~n"
    "  4> poolboy_bench:bench(#{pool_sizes => [2,4,8,16,32,64]}).~n"
    "~n"
    "  $ make bench-poolboy~n"
    "  $ make bench-poolboy BENCH_OPTS='#{pool_sizes => [2,4,8,16]}'~n"
  ),
  ok.
...
bench_scaling(PoolSizes, Opts) ->
  io:format("~n=== Poolboy Scaling Analysis ===~n"),
  ...
  io:format("~n┌───────────┬─────────────┬──────────────────────┐~n"),
  io:format("│ Pool Size │   Poolboy   │      Rejected        │~n"),
...
capture_single_bench(Opts) ->
  ...
  try
    ...
    Metrics = report_metrics(PoolSize, PoolStrategy, Workers, ElapsedMs, AllLatenciesUs, Rejected),
    {ok, Metrics}
  catch
    Class:Reason ->
      {error, {Class, Reason}}
...
report_metrics(PoolSize, PoolStrategy, Workers, ElapsedMs, LatenciesUs, Rejected) ->
  N = length(LatenciesUs),
  Sorted = lists:sort(LatenciesUs),
  ThroughputPerSec = N * 1000 / ElapsedMs,
